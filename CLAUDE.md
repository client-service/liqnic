...
`docker-compose.yml` runs all three services with Traefik labels for HTTPS termination. Backend static file uploads are served from the `static/` directory via `MEDUSA_SERVE_STATIC_URL`. Redis is required for production — all four Redis env vars (`REDIS_URL`, `CACHE_REDIS_URL`, `EVENTS_REDIS_URL`, `LOCKING_REDIS_URL`) must be set.
