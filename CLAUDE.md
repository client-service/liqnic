...
Each service's CircleCI workflow is split into a `-dev` and `-prod` copy (e.g. `storefront-workflow-dev` / `storefront-workflow-prod`), gated by branch so the `liqnic-dev` and `liqnic-prod` contexts are never both attached to the same job. A manual edit to `.env`/`.env.backend` on the server needs `deploy/restart.sh` (or `docker compose up -d --force-recreate <service>`) to take effect — editing the file alone doesn't restart the container or re-read it.

See `deploy/README.md` for the full breakdown of which server-side files exist, how each service's env vars get populated (build-time vs. deploy-time vs. runtime), and where each value's source of truth lives (CircleCI context vs. server-only file).
